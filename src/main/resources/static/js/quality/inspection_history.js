document.addEventListener('DOMContentLoaded', function() {
	...
	class HistoryController {
		...
		initializeCharts() {
			...
			this.pieChartContext = document.getElementById('inspectionPieChart').getContext('2d');
			...
			this.barChartContext = document.getElementById('processBarChart').getContext('2d');
			this.barChart = new Chart(this.barChartContext, {
				type: 'bar',
				data: {
					labels: [],
					datasets: [
		...
		updateCharts(stats) {
			// Update pie chart
			this.pieChart.data.datasets[0].data = [stats.passCount, stats.failCount];
			this.pieChart.update();

			// Update bar chart with process stats
			if (stats.processStats && stats.processStats.length > 0) {
				const processNames = stats.processStats.map(ps => ps.processName);
				const passCounts = stats.processStats.map(ps => ps.passCount);
				const failCounts = stats.processStats.map(ps => ps.failCount);

				this.barChart.data.labels = processNames;
				this.barChart.data.datasets[0].data = passCounts;
				this.barChart.data.datasets[1].data = failCounts;
		...
		updateStats(stats) {
		       ...
		       // Update charts with the statistics data
